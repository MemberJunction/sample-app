...
export class RunViewResolver extends ResolverBase {
  ...
  async RunViewByName(
    ...
    try {
      ...
      const entityId = await dataSource.query(`SELECT EntityID from vwUserViews WHERE Name='${input.ViewName}'`);
  ...
  async RunViewByID(
    ...
    try {
      ...
      const entityId = await dataSource.query(`SELECT EntityID from vwUserViews WHERE ID=${input.ViewID}`);
      const returnData = this.processRawData(rawData.Results, entityId[0].EntityID);
      return {
        Results: returnData,
        UserViewRunID: rawData?.UserViewRunID,
  ...
  async RunDynamicView(
    ...
    try {
      ...
      const entityId = await dataSource.query(`SELECT ID from vwEntities WHERE Name='${input.EntityName}'`);
      const returnData = this.processRawData(rawData.Results, entityId[0].EntityID);
      return {
        Results: returnData,
        UserViewRunID: rawData?.UserViewRunID,
